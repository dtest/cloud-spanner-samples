...
final class SpannerDaoPGImpl implements SpannerDaoInterface {
      ...
      this.databaseClient
          ...
          .run(transaction -> {
            Statement statement = Statement.newBuilder(
                ...
                .bind("p3")
                .to(Value.pgNumeric(balance.toString())) // Numeric requires special handling
                .build();
            transaction.executeUpdate(statement);
      ...
      return this.databaseClient.readWriteTransaction()
          .run(transaction -> {
            ImmutableMap<ByteArray, AccountData> accountData =
                readAccountDataForTransfer(ImmutableList.of(fromAccountId, toAccountId), transaction);
  ...
  private void updateAccount(ByteArray accountId, BigDecimal newBalance, TransactionContext transaction)
      throws SpannerException {
    try {
      Statement statement = Statement.newBuilder(
              "UPDATE Account SET Balance = $1 WHERE AccountId = $2")
          .bind("p1")
  ...
  private void insertTransferTransactions(
      ByteArray fromAccountId, ByteArray toAccountId, BigDecimal amount, TransactionContext transaction)
      throws SpannerException {
    try {
      Statement statement = Statement.newBuilder(
